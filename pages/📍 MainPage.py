...
if(logged_in()[0]):
    ...
    selected_convo = st.sidebar.radio("Choose conversation:", sorted(st.session_state.collections))
    ...
    with st.form("displayChat"):
        clear_btn = st.form_submit_button(f"======= CHAT LOG {selected_convo} =======", disabled=True)
        collection_ref = db.collection(selected_convo)
        documents = collection_ref.order_by("timestamp", direction=firestore.Query.DESCENDING).get()
